    ...
    for item_id, snt, mrss in items:
        ...
        #print(item_j+ ',')
         for mrs in mrss[:args.n]:
             try:
                 make_penman(snt, item_id, mrs, args)
...
def make_penman(snt, item_id, x, args):
    g = make_graph(x, args)
    dic = g.to_dict()
    # Write Sentence
    with open('data/test/sentence/'+item_id+'.txt', 'w+') as f:
        f.write(snt)
    # Write penman reprentation
    with open('data/test/penman/'+item_id+'.pickle', 'wb') as f:
